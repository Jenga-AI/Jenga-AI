...
class MultiHeadQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Labels are already clean dicts with proper structure
...
def train_model(config):
    ...
    logging.info(f"Loading data from {config['data']['train_data_path']}")
    df = pd.read_json(config['data']['train_data_path'])
    
    # Data is pre-cleaned - no parsing or normalization needed!
    logging.info(f"Loaded {len(df)} pre-cleaned samples")
    ...
    # Save test split
    os.makedirs(os.path.dirname(config['data']['test_data_path']), exist_ok=True)
    ...
    epochs_no_improve = 0
    with mlflow.start_run(run_name="QA Model Training Test") as run:
        ...
        if config.get('output', {}).get('save_final_model', True):
            ...
            logging.info(f"Training complete! Final model saved to {final_output_dir}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train QA Multi-Head Model')
